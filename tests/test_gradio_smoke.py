...
class FakePlainTextOrchestrator:
    def __init__(self) -> None:
        self.last_kwargs: dict[str, object] = {}
        ...
        return FullPipelineResult(
            ...
            documents=[
                OCRDocumentStageResult(
                    ...
                    ocr_status="ok",
                    pages_count=2,
                    combined_markdown_path="/tmp/doc-plain/combined.md",
                    ocr_artifacts_path="/tmp/doc-plain/ocr",
                    ocr_error=None,
                )
            ],
            critical_gaps_summary=[],
            next_questions_to_user=[],
            raw_json_text="Plain text legal brief",
            parsed_json=None,
            validation_valid=True,
            validation_errors=[],
            metrics={"usage": {"total_tokens": 10}},
...
def test_app_css_enables_resizable_raw_response_box() -> None:
    assert "#raw_json_box textarea" in gradio_app_module._APP_CSS
    assert "resize: vertical" in gradio_app_module._APP_CSS
    assert "min-height: 18rem" in gradio_app_module._APP_CSS
...
def test_run_full_pipeline_ui_handles_plain_text_prompt(
    tmp_path: Path,
) -> None:
    file_one = tmp_path / "one.pdf"
    file_two = tmp_path / "two.pdf"
    ...
    orchestrator = FakePlainTextOrchestrator()
    (
        status,
        session_state,
        run_id,
    ...
    assert "run-plain" in status
    assert session_state == "session-plain"
    assert run_id == "run-plain"
    assert session_id == "session-plain"
    assert artifacts_root == ""
    assert error_user == "No errors."
    assert error_details == ""
    assert ocr_rows[0][0] == "0000001"
    ...
    assert gap_rows == []
    assert details_text == "Details are unavailable for plain-text prompt."
    assert summary == "Plain text legal brief"
    assert raw_json == "Plain text legal brief"
    assert validation == "Validation: skipped (plain-text prompt)"
    assert "total_tokens" in metrics
...
def test_history_load_handles_plain_text_prompt_bundle(
    tmp_path: Path,
) -> None:
    repo = StorageRepo(db_path=tmp_path / "kaucja.sqlite3")
    session = repo.create_session("session-history-plain")
    run = repo.create_run(
        session_id=session.session_id,
        provider="openai",
        model="gpt-5.4",
        ...
        json.dumps(
            {
                ...
                "status": "skipped",
                "response_mode": "plain_text",
                "note": "Validation skipped for plain-text prompt.",
            }
        ),
        encoding="utf-8",
    )
    ...
    init_run_manifest(
        ...
        inputs={
            ...
            "prompt_version": run.prompt_version,
            "schema_version": run.schema_version,
            "prompt_response_mode": "plain_text",
            "ocr_params": {"model": "mistral-ocr-latest"},
            "llm_params": {"openai_reasoning_effort": "auto"},
        },
        artifacts={
    ...
    assert raw_json == "Plain text legal brief"
    assert (
        validation == "Validation: skipped (Validation skipped for plain-text prompt.)"
    )
    assert "total_tokens" in metrics
    assert parsed_state is None
...
def test_on_provider_change_ui_includes_gpt_5_4_for_openai() -> None:
    from app.ui.gradio_app import on_provider_change_ui

    result = on_provider_change_ui("openai")

    assert "gpt-5.1" in result["choices"]
    assert "gpt-5.2" in result["choices"]
    assert "gpt-5.4" in result["choices"]
