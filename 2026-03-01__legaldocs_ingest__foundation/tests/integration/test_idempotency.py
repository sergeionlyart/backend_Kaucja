...
from legal_ingest.store.models import Document, DocumentSource, Page, Node, PageExtraction, PageExtractionQuality, ContentStats
...
def test_idempotent_upsert(mongo_cfg):
    ...
    page = Page(
        ...
        token_count_est=2,
        char_count=9,
        extraction=PageExtraction(method="HTML", quality=PageExtractionQuality(alpha_ratio=1.0, empty=False))
    )
    ...
    doc = Document(
        ...
        mime="text/html",
        page_count=1,
        content_stats=ContentStats(total_chars=9, total_tokens_est=2, parse_method="HTML", ocr_used=False)
    )
